CLASS zclpp_kg_fat_snf_reprocess DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    DATA : p_plant TYPE c LENGTH 4,
           ...
           s_rawm  TYPE RANGE OF matnr,
           l_rawm  LIKE LINE OF s_rawm.
...
CLASS ZCLPP_KG_FAT_SNF_REPROCESS IMPLEMENTATION.
  ...
  METHOD if_apj_dt_exec_object~get_parameters.
    ...
    et_parameter_def = VALUE #(
      ...
      ( selname = 'S_RAWM' kind = if_apj_dt_exec_object=>select_option
        datatype = 'C' length = 40
        mandatory_ind = abap_true
    ...
    et_parameter_val = VALUE #(
      ...
      ( selname = 'S_RAWM' kind = if_apj_dt_exec_object=>parameter sign = 'I' option = 'EQ' low = '000000000000000033' )
  ...
  METHOD if_apj_rt_exec_object~execute.
    ...
    LOOP AT it_parameters INTO DATA(ls_parameter).
      CASE ls_parameter-selname.
        ...
        WHEN 'S_RAWM'.
          l_rawm-sign = ls_parameter-sign.
          l_rawm-option = ls_parameter-option.
          l_rawm-low = ls_parameter-low.
          l_rawm-high = ls_parameter-high.
          APPEND l_rawm TO s_rawm.
          CLEAR l_rawm.
        WHEN 'P_ORDM'.
...
*WHERE  Plant = @p_plant
*AND   StorageLocation = @p_sloc
*and   Material IN @S_RAWM "'000000000000000033'
...
*into TABLE @data(lt_stock).
        ...
        WHERE a~manufacturingordertype = 'YQ10' "'YBM2'
              ...
              IF lv_snf IS NOT INITIAL AND
                ...
                IF wa_log-gr_doc IS INITIAL.
                  ...
                  LOOP AT lt_res INTO DATA(ls_res) WHERE goodsmovementtype = 101.
                    ...
                    CLEAR lv_qty_1.
                    IF ls_stock-materialbaseunit NE 'L'.
                      SELECT SINGLE alternativeunit, quantitynumerator,quantitydenominator
                        FROM i_productunitsofmeasure WITH PRIVILEGED ACCESS
                        WHERE alternativeunit = 'L'
                        AND product = @ls_stock-material
                        AND baseunit = @ls_stock-materialbaseunit
                        INTO @DATA(lw_base_unit).

                      lv_qty_1 = ls_stock-stock * ( lw_base_unit-quantitydenominator / lw_base_unit-quantitynumerator ).
                    ELSE.
                      lv_qty_1 = ls_stock-stock.
                    ENDIF.
                    ...
                    IF lv_cnt = 1.
* lv_qty = ( lv_qty_1 * '1.030' ) * lv_fat / 100.
                      lv_qty = ( lv_qty_1 * lv_volume-sizeordimensiontext ) * lv_fat / 100.
                    ELSE.
* lv_qty = ( lv_qty_1 * '1.030' ) * lv_snf / 100.
                      lv_qty = ( lv_qty_1 * lv_volume-sizeordimensiontext ) * lv_snf / 100.
                    ENDIF.
