CLASS zbg_loading_sheet DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
   INTERFACES if_serializable_object .
    ...
    METHODS constructor
      IMPORTING
        iv_truck_sheet TYPE  zde_trucksheet_no.
  PROTECTED SECTION.
  PRIVATE SECTION.
  DATA : im_truck_sheet      TYPE zde_trucksheet_no.
          METHODS modify
      RAISING
        cx_bgmc_operation.
ENDCLASS.
...
CLASS ZBG_LOADING_SHEET IMPLEMENTATION.


  METHOD constructor.
  im_truck_sheet = iv_truck_sheet.
...
*      out->write( 'Template Store Client initialized' ).
        ...
        DATA(lo_fdp_util) = cl_fp_fdp_services=>get_instance( 'ZSD_LOADING_SHEET_FORM' ).
...
*      out->write( 'Service data retrieved' ).
        ...
        DATA(ls_template) = lo_store->get_template_by_name(
          ...
          iv_form_name      = 'LOADING_SHEET' "<= form object in template store
          iv_template_name  = 'LOADING_SHEET' "<= template (in form object) that should be used
        ).
...
*      out->write( 'Form Template retrieved' ).
        ...
        DATA : wa_data TYPE zsdt_load_sheet .
        wa_data-trucksheet_no = im_truck_sheet.

        wa_data-pdf_data = lv_pdf.
        MODIFY zsdt_load_sheet FROM @wa_data.
