CLASS zcl_einvoice_1 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZCL_EINVOICE_1 IMPLEMENTATION.
  ...
  METHOD call_api.
    ...
    DATA:  lv_content_length_value   TYPE i,
           ...
           lv_SignedQRCode          TYPE string,
           lv_status                TYPE string,
           WA_zsdt_invrefnum        TYPE zsdt_invrefnum.
...
*
**    SELECT * FROM zsdt_invrefnum INTO TABLE @DATA(it_zsdt_invrefnum)
**     WHERE docno      = @wa_vbrk-vbeln.
*
**    CLEAR : lv_count, i_date, i_fyv, e_fy.
**    lv_count = lines( it_j_1ig_invrefnum ).
**
**    lv_count = lv_count + 1.
*
*    wa_zsdt_invrefnum-bukrs      = wa_vbrk-bukrs.
*    wa_zsdt_invrefnum-docno      = wa_vbrk-vbeln.
*
...
**    ENDIF.
*
*    wa_zsdt_invrefnum-doc_year   = e_fy.
*    wa_zsdt_invrefnum-doc_type   = wa_vbrk-fkart.
*    wa_zsdt_invrefnum-odn        = wa_vbrk-xblnr.
*    wa_zsdt_invrefnum-odn_date   = wa_vbrk-fkdat.
...
*    wa_zsdt_invrefnum-irn_status = 'ACT'. "GS_RESP_POST-RESPONSE-STATUS.
**  WA_J_1IG_INVREFNUM-CANCEL_DATE
*    wa_zsdt_invrefnum-ernam      = sy-uname.
*    wa_zsdt_invrefnum-erdat      = sy-datum.
*    wa_zsdt_invrefnum-erzet      = sy-uzeit.
*    wa_zsdt_invrefnum-signed_inv = lv_signedinvoice.
*
*    IF lv_signedinvoice IS NOT INITIAL.
*      wa_zsdt_invrefnum-signed_qrcode = lv_signedqrcode.
*    ELSEIF gs_resp_post-response-qrcode IS NOT INITIAL.
*      wa_zsdt_invrefnum-signed_qrcode = lv_signedqrcode.
*    ENDIF.
*
...
***  REPLACE ALL OCCURRENCES OF '":"0.00",' IN lv_json WITH '":null,'.
  ...
  DATA : LV_STATE_CD(2) TYPE C.
    ...
    lv_vbeln = 'B910000005'. "'0090000007'.
...
*     IF SY-sysid = 'ZJM' OR SY-sysid = 'ZXP'.
      gv_gstin_seller =  wa_sellerdtls-gstin = '27AAFCD5862R013'. "wa_j_1bbranch-gstin.
...
*      \_standardaddress-transportzone,
*      \_standardaddress-addresscitypostboxcode,
*      \_standardaddress-person FROM i_customer
